function load_list_transaction_all(page) {
  ...
  app.calendar.create({
    inputEl: '#txtsearchdatetransaction',
    openIn: 'customModal',
    header: true,
    footer: true,
  });
  ...
  $$('#category_transaction_selection').on('change', function () {
    var username = $$('#txtsearchdatetransaction').val();
  ...
  $$('#txtsearchdatetransaction').on('change', function() {
    var username = $$('#txtsearchusertransaction').val();
    var date = $$('#txtsearchdatetransaction').val();
    var category = $$('#category_transaction_selection').val();

    app.request({
      method: "POST",
      url: database_connect + "transaction/select_transaction_all.php", data:{ transaction_type : category, username : username, date : date },
      success: function(data) {
        var obj = JSON.parse(data);
        if(obj['status'] == true) {
          var total = formatRupiah(obj['total']);
